...
//   // New state for Summary popup and incorrect answers
...
//           return;
//         }
//         console.log("Calling API for the first time");
...
//     if (!selectedAnswer || !quizData || !sessionId) {
...
//     try {
//       const token = sessionStorage.getItem("token");
//       const selectedIndex = quizData.options.indexOf(selectedAnswer);
//       console.log("Calling second API");
//       const response = await axios.post(
//         "/api/v1/mcq/submit",
//         { action: selectedIndex.toString() },
//         {
//           headers: {
//             Authorization: `Bearer ${token}`,
//             "Content-Type": "application/json",
...
//       console.log("2nd api points = ", response.data.points, " and done ", response.data.done);

//       const isCorrect = response.data.is_correct;
//       setLastAnswerCorrect(isCorrect);
//       if (!isCorrect && response.data.correct_answer) {
...
//   // When closing the Congratulations popup, fetch incorrect answers and show Summary popup
//   const handleCloseCompletionPopup = async () => {
//     setQuizCompleted(false);
//     try {
//       const token = sessionStorage.getItem("token");
//       console.log("Fetching incorrect answers...");
...
//         },
//       });
//       console.log("Incorrect answers response:", response.data);
//       // Adjust based on the API response property name
...
//   // Close the Summary popup and navigate to /home
...
//           ? baseClass + "border-2 border-[#1EFF00] bg-white text-[#1EFF00]"
//           : baseClass + "border-2 border-[#DB0000] bg-white text-[#DB0000] animate-shake";
//       }
//     }
...
//       <audio ref={incorrectSoundRef} src="/sounds/incorrect.mp3" preload="auto"></audio>
//       <audio ref={pointsSoundRef} src="/sounds/points.mp3" preload="auto"></audio>
//       <audio ref={completionSoundRef} src="/sounds/completion.mp3" preload="auto"></audio>
...
//               exit={{ opacity: 0 }}
//             >
//               <div className="text-5xl font-bold text-green-500 flex items-center">
...
//               >
//                 {[...Array(Math.min(5, pointsGained))].map((_, i) => (
//                   <motion.div
//                     key={i}
...
//         {/* Summary Popup for Incorrect Answers */}
//         <AnimatePresence>
//           {showSummary && (
//             <motion.div
//               className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50"
...
//                 </h2>
//                 {/* Scrollable container for a potentially large response */}
...
function MCQQuiz() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const isCorrect = response.data.is_correct;
      setLastAnswerCorrect(isCorrect);
      setCorrectAnswer(isCorrect ? null : response.data.correct_answer);
  ...
  const getOptionClassName = (option) => {
    let baseClass = "p-10 text-[16px] leading-[24px] font-medium font-poppins rounded-[20px] shadow-sm shadow-[#B120301A] text-center border transition-all ";
    if (!showResult && selectedAnswer === option) return baseClass + "border-2 border-black";
    if (showResult) {
      if (option === correctAnswer) return baseClass + "border-2 border-[#1EFF00] bg-white text-[#1EFF00]";
      if (selectedAnswer === option) return lastAnswerCorrect ? 
        baseClass + "border-2 border-[#1EFF00] bg-white text-[#1EFF00]" : 
        baseClass + "border-2 border-[#DB0000] bg-white text-[#DB0000] animate-shake";
    }
    return baseClass + "border-gray-200 hover:border-gray-300";
  ...
  return (
    <div className="min-h-screen bg-white p-4 md:p-6 overflow-hidden">
      ...
      <button onClick={toggleSound} className="fixed top-4 right-4 p-2 bg-white rounded-full shadow-md z-10">
        {soundEnabled ? (
        ...
        {showPointsAnimation && (
          <motion.div className="fixed inset-0 pointer-events-none z-50 flex items-center justify-center"
            ...
            <div className="relative w-full h-full overflow-hidden">
              {[...Array(30)].map((_, i) => (
                <motion.div key={`star-${i}`} className="absolute text-2xl"
                  style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%` }}
                  variants={floatingStars} initial="initial" animate="animate">⭐</motion.div>
              ))}
            </div>

            <motion.div className="flex flex-col items-center z-50"
              initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: 'spring', stiffness: 300 }}>
              <motion.div className="bg-gradient-to-r from-yellow-400 to-red-500 rounded-full p-4 shadow-xl"
                variants={scaleUp} initial="initial" animate="animate">
                <motion.div className="text-6xl font-bold text-white flex items-center" variants={pointsBounce}>
                  <motion.span className="mr-2">🎉</motion.span>
                  <motion.span className="block" animate={{ scale: [1, 1.2, 1], rotate: [0, 10, -10, 0] }}
                    transition={{ duration: 0.5, repeat: Infinity, repeatType: 'mirror' }}>+{pointsGained}</motion.span>
                  <motion.span className="ml-2">🏆</motion.span>
                </motion.div>
              </motion.div>

              <motion.div className="mt-4 text-2xl font-semibold text-yellow-400"
                initial={{ opacity: 0 }} animate={{ opacity: [0, 1, 0], scale: [0.8, 1.2, 1] }}
                transition={{ duration: 1.5, times: [0, 0.5, 1] }}>Keep it up!</motion.div>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <div className="bg-white rounded-full border border-[#B12030] p-1.5 flex flex-col sm:flex-row gap-2">
          <div className="flex">
            <button className="bg-red-700 text-white rounded-full py-3 px-5 flex items-center gap-4">
              <div className="flex items-center gap-3">
                <div className="rounded-full flex items-center justify-center">
                  <svg width="24" height="32" viewBox="0 0 20 28" fill="none" xmlns="http://www.w3.org/2000/svg"></svg>
                </div>
                <div className="text-left">
                  <div className="font-semibold text-[12px] leading-[18px] font-poppins">Current Role</div>
                  <div className="font-medium text-[10px] leading-[15px] font-poppins">Software Engineer</div>
                </div>
              </div>
              <ChevronDown className="h-5 w-5" />
            </button>
          ...
          <div className="flex">
            <button className="bg-white rounded-full py-3 px-5 flex items-center gap-10 hover:bg-gray-50 transition-colors">
              <div className="flex items-center gap-3">
                <div className="rounded-full flex items-center justify-center">
                  <svg width="21" height="27" viewBox="0 0 21 27" fill="none" xmlns="http://www.w3.org/2000/svg"></svg>
                </div>
                <div className="text-left">
                  <div className="font-semibold text-[12px] leading-[18px] font-poppins">Senior Role</div>
                  <div className="font-semibold text-[10px] leading-[15px] font-poppins">Senior Software Engineer</div>
                </div>
              </div>
              <ChevronDown className="h-5 w-5 text-black" />
            </button>
      ...
      <div className="max-w-[1167px] mx-auto mb-8">
        <div className="flex justify-between items-center max-w-[862px] mx-auto border-b border-gray-200 pb-4 mb-8">
          <div className="flex items-center">
            <span className="font-medium text-gray-700 mr-2">Level:</span>
            <motion.span className="font-bold text-red-700" key={levelString}
          ...
          <div className="flex items-center">
            <span className="font-medium text-gray-700 mr-2">Points:</span>
            <motion.span className="font-bold text-red-700" key={points}
              initial={{ scale: 1 }} animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 0.5 }}>
              {points}
            </motion.span>
          </div>
        </div>
      ...
      <div className="max-w-[1167px] mx-auto">
        <motion.div className="p-[50px] max-w-[862px] border border-[#B120301A]/10 rounded-[40px] mx-auto shadow-md shadow-[#B120301A]/10 mb-10"
          initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} key={quizData.question}>
          <div className="text-[22px] leading-[33px] font-medium font-poppins text-[#564D4D] text-center mb-8">
            {quizData.question}
          </div>
        </motion.div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
          {quizData.options.map((option, index) => (
            <motion.button key={index} onClick={() => handleAnswerSelect(option)}
              className={getOptionClassName(option)} disabled={showResult}
              initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.3, delay: 0.1 * index }} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
              <div className="flex items-center justify-center gap-2">
                <span>{option}</span>
              </div>
            </motion.button>
        ...
        {!showResult && (
          <div className="flex justify-center mb-8">
            <motion.button onClick={handleSubmit} disabled={!selectedAnswer}
              className="bg-red-600 text-white px-6 py-2 rounded-full hover:bg-red-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
        ...
        <AnimatePresence>
          {showResult && !quizCompleted && (
            <motion.div className={`fixed bottom-0 left-0 right-0 h-[150px] text-white px-4 py-12 ${lastAnswerCorrect ? "bg-green-600" : "bg-red-600"}`}
              initial={{ y: 150, scale: 0.8 }} animate={{ y: 0, scale: 1 }} exit={{ y: 150 }}
              transition={{ type: "spring", stiffness: 300, damping: 15 }}>
              <div className="max-w-7xl mx-auto flex items-center justify-between">
                <div className="flex items-center gap-3">
                  {lastAnswerCorrect ? (
                    <>
                      <motion.div className="bg-green-500 rounded-full p-2" initial={{ scale: 0 }} animate={{ scale: [0, 1.2, 1] }} transition={{ duration: 0.5 }}>
                        <Check className="h-6 w-6" />
                      </motion.div>
                      <div>
                        <div className="text-green-400 font-bold">Correct :)</div>
                    ...
                    <>
                      <motion.div className="rounded-full p-2" initial={{ rotate: 0 }} animate={{ rotate: [0, -10, 10, -10, 0] }} transition={{ duration: 0.5 }}>
                ...
                <motion.button onClick={handleNext} className="bg-red-600 text-white px-4 py-2 rounded-full hover:bg-red-700 transition-colors"
                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                  Next Question
                </motion.button>
              </div>
        ...
        <AnimatePresence>
          {quizCompleted && (
            <motion.div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
              <motion.div className="bg-white rounded-lg p-8 max-w-md w-full text-center"
                ...
                <div className="mb-4">
                  <motion.div className="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4"
                    initial={{ scale: 0 }} animate={{ scale: [0, 1.2, 1] }} transition={{ duration: 0.6, delay: 0.2 }}>
                    <Check className="h-10 w-10 text-green-500" />
                  </motion.div>
                  ...
                  <motion.p className="text-gray-600 mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.6 }}>
                    You’ve completed the quiz successfully!
                  </motion.p>
                  <motion.p className="font-bold text-xl text-red-600 mb-2" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.8 }}>
                    Final Score: {points} points
                  </motion.p>
                  <motion.p className="font-medium text-gray-700 mb-6" initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1 }}>
                    Level achieved: {levelString}
                  </motion.p>
                </div>
                <motion.button onClick={handleCloseCompletionPopup} className="bg-red-600 text-white px-6 py-3 rounded-full hover:bg-red-700 transition-colors w-full font-medium"
                  whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 1.2 }}>
                  Continue
                </motion.button>
              </motion.div>
            </motion.div>
          ...
          {showSummary && (
            <motion.div className="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-50"
              ...
              <motion.div className="bg-white rounded-lg p-8 max-w-md w-full text-center"
                ...
                <button onClick={handleCloseSummary} className="bg-red-600 text-white px-6 py-3 rounded-full hover:bg-red-700 transition-colors w-full font-medium mt-4">
                  Close
                </button>
              </motion.div>
            </motion.div>
